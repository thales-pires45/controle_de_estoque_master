...
def relatorio_faturamento(request):
    if request.user.is_authenticated:
        ...
        for i in range(12):
            ...
            y = venda_user.filter(data__gte=start_date, data__lt=end_date).aggregate(Sum('valor_total'))['valor_total__sum']
            if y is None:
                y = 0
...
def relatorio_produtos(request):
    ...
    # Retorna os 3 mais vendidos
    return JsonResponse({'labels': labels[:3], 'data': data[:3]})
